...
<style>
  ...
  .back { display: inline-flex; margin-bottom: .85rem; color: var(--accent); text-decoration: none; font-weight: 850; }
  .hero { padding: 1.25rem; margin-bottom: 1.3rem; border-radius: 1.25rem; color: var(--txt); background: radial-gradient(circle at top left, rgba(59,130,246,.3), transparent 38%), var(--card); border: 1px solid var(--border); }
  .hero span,
  .kicker { color: var(--accent); text-transform: uppercase; letter-spacing: .07em; font-size: .72rem; font-weight: 850; }
  .hero h1 { margin: .35rem 0; color: var(--txt); font-size: clamp(2rem, 7vw, 3.2rem); line-height: 1; }
  .hero p,
  .section-head p,
  ...
  .section-head h2 { margin: 0 0 .2rem; color: var(--txt); font-size: 1.25rem; }
  .section-head strong { color: var(--accent); white-space: nowrap; }
  .grid { display: grid; grid-template-columns: minmax(0,1fr); gap: .85rem; }
  .card { display: grid; grid-template-columns: auto 1fr; gap: .85rem; min-height: 132px; padding: 1rem; color: var(--txt); text-decoration: none; background: linear-gradient(135deg, color-mix(in srgb, var(--subject-color) 20%, transparent), transparent 52%), var(--card); border: 1px solid var(--border); border-radius: 1rem; transition: transform 120ms ease, background 120ms ease, border-color 120ms ease; }
  .card:hover,
  .card:focus-visible { transform: translateY(-1px); background: linear-gradient(135deg, color-mix(in srgb, var(--subject-color) 26%, transparent), transparent 55%), var(--card-hover); border-color: color-mix(in srgb, var(--subject-color) 45%, var(--border-strong)); outline: none; }
  .icon { font-size: 1.7rem; }
  .card h3 { margin: .12rem 0 .25rem; color: var(--txt); font-size: 1.05rem; }
