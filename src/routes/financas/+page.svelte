  ...
  {:else}
    ...
    <section class="quick-links" aria-label={$t('financas.shortcuts.aria', { default: 'Atalhos' })}>
      ...
      <a class="quick" href="/financas/relatorios/">
        <span class="quick-icon" aria-hidden="true">📊</span>
        <span class="quick-text">
          <span class="quick-title">{$t('financas.shortcuts.reports.title', { default: 'Relatórios' })}</span>
          <span class="quick-sub">{$t('financas.shortcuts.reports.sub', { default: 'Pie · top 5 · comparativo · CSV' })}</span>
        </span>
      </a>
    </section>
  {/if}
